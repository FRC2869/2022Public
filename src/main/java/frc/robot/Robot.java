...
public class Robot extends TimedRobot {
	...
	public void robotInit() {
		...
		// ShuffleboardTab test = Shuffleboard.getTab("test");
		// closeEntry = test.addPersistent("closeentry",
		// 8000).withWidget(BuiltInWidgets.kNumberSlider).withProperties(Map.of("min",
		// 0, "max", 10000)).withPosition(0,0).withSize(3, 1).getEntry();
		// closeAngleEntry = test.addPersistent("close angle",
		// 70).withWidget(BuiltInWidgets.kNumberSlider).withProperties(Map.of("min", 0,
		// "max", 85)).withPosition(0,1).withSize(3, 1).getEntry();
		...
		// autoPickerEntry = auto.add("autoselector",
		// driveBack).withWidget(BuiltInWidgets.kComboBoxChooser).withPosition(0,
		// 0).withSize(3, 1).getEntry();
		// auto.add("auto", autopicker).withPosition(0, 0).withSize(3, 1);

		newautopick.addOption("fender", automodes.fender);
		newautopick.addOption("back", automodes.backupOnly);
		newautopick.setDefaultOption("fender preset", automodes.fenderpreset);
		...
		// autorotateEntry = auto.add("autorotate", autorotate).withPosition(0,
		// 0).withSize(1, 1).getEntry();
		...
		angletolerance = auto.addPersistent("angle tolerance", angletol).withPosition(0, 3).withSize(3, 1)
				.withWidget(BuiltInWidgets.kNumberSlider).withProperties(Map.of("min", 0, "max", 5)).getEntry();
		powerRevEntry = auto.addPersistent("power rev", powerRev).withPosition(0, 4).withSize(3, 1)
				.withWidget(BuiltInWidgets.kNumberSlider).withProperties(Map.of("min", 0, "max", 2)).getEntry();
		...
		// autoPickerEntry = auto.add("autoselector",
		// autopicker).withWidget(BuiltInWidgets.kComboBoxChooser).withPosition(0,
		// 0).withSize(3, 1).getEntry();
		// auto.add("autoselector", 0);

		ShuffleboardTab teleop = Shuffleboard.getTab("Teleop");
		distanceEntry = teleop.add("Distance", 0.0).withWidget(BuiltInWidgets.kNumberSlider)
				.withProperties(Map.of("min", 1, "max", 7)).withPosition(0, 0).withSize(3, 1).getEntry();
		velconstantEntry = teleop.addPersistent("velconstant", 6.78).withWidget(BuiltInWidgets.kNumberSlider)
				.withProperties(Map.of("min", 6, "max", 10)).withPosition(0, 1).withSize(3, 1).getEntry();
		launchvelconstantEntry = teleop.addPersistent("launchvelconstant", 1).withWidget(BuiltInWidgets.kNumberSlider)
				.withProperties(Map.of("min", 0.5, "max", 1.5)).withPosition(0, 2).withSize(3, 1).getEntry();
		slider775 = teleop.addPersistent("slider775", 1).withWidget(BuiltInWidgets.kNumberSlider)
				.withProperties(Map.of("min", 5, "max", 12)).withPosition(0, 3).withSize(3, 1).getEntry();
		...
		// leftFalEntry = teleop.add("lfalc", 0).withPosition(0, 4).withSize(1,
		// 1).getEntry();
		// rightFalEntry = teleop.add("rfalc", 0).withPosition(1, 4).withSize(1,
		// 1).getEntry();
		...
		// tarspeedthing = teleop.add("tarspeedthing", 7000).withPosition(4,
		// 4).withWidget(BuiltInWidgets.kNumberSlider).withProperties(Map.of("min",
		// 7000, "max", 9000)).withSize(3, 1).getEntry();
	...
	public void robotPeriodic() {
		...
		time.setNumber(ShooterSubSys.calcVel());
		ballinEntry.setBoolean(!ShooterSubSys.isIntakeEmpty());
	...
	public void disabledInit() {
		...
		RobotContainer.drivetrainSubSys.coast();
		// offFlash();
		// networkInit();
		// offLime();
	}
	...
	public void autonomousInit() {
		...
		RobotContainer.drivetrainSubSys.brake();
		// photonResetPipe();
		Shuffleboard.selectTab("Auto");
	...
	public void teleopInit() {
		...
		RobotContainer.angleSubSys.setTargetLow();
		// initLime();
		...
		// try{
		// HttpCamera ll = new HttpCamera("limelight",
		// "http://limelight.local:5800/stream.mjpeg"); // TODO check this
		...
		// DifferentialDriveOdometry odom = new DifferentialDriveOdometry(Rotation2d.fromDegrees(0), new Pose2d());
		// System.out.println(odom.getPoseMeters());
		// odom.update(Rotation2d.fromDegrees(0), 0, 0);
		// System.out.println(odom.getPoseMeters());
		// odom.update(Rotation2d.fromDegrees(0), 1, -1);
		// System.out.println(odom.getPoseMeters());
		// odom.update(Rotation2d.fromDegrees(0), 2, -2);
		// System.out.println(odom.getPoseMeters());
	}
	...
	public void teleopPeriodic() {
		// autoSchedule(RobotContainer.gotopos);
		
		
		if (getFenderPressed()) {
			RobotContainer.fenderShot = new FenderShot();
			autoSchedule(RobotContainer.fenderShot);
		} else if (getTarmacPressed()) {
			RobotContainer.tarmacShot = new TarmacShot();
			autoSchedule(RobotContainer.tarmacShot);
		} 
	else if ((RobotContainer.fenderShot == null || !RobotContainer.fenderShot.isScheduled()) 
	&& (RobotContainer.tarmacShot == null || !RobotContainer.tarmacShot.isScheduled())) {
			...
			// 	// } else if (!RobotContainer.pointIntake.isScheduled() &&
			// 	// !RobotContainer.autoLift.isScheduled() &&
			// 	// !RobotContainer.autoDown.isScheduled() &&
			// 	// !RobotContainer.autoMid.isScheduled() && !isClose()){
			} else if (!RobotContainer.autoLift.isScheduled() && !RobotContainer.autoDown.isScheduled() 
					&& !RobotContainer.autoMid.isScheduled() && !RobotContainer.autoClose.isScheduled()) {
				autoSchedule(RobotContainer.shooter);
		...
		// distanceEntry.setDouble(LimelightSubSys.getDistance());
